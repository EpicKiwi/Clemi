...
const { string, TYPES_EQUIVALENTS } = require("./PropParsers");
...
class Component extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (props) {
      Object.keys(props).forEach(key => {
        if (!props[key].callback) return;
        let callback = this[props[key].callback];
        if (!callback)
          return console.warn(
            `${this.constructor.name} : No callback named ${
              props[key].callback
            } on property ${key}`
          );
  ...
  $mountShadowTemplate() {
    ...
    let template =
      typeof this.constructor.template === "string"
        ? html([this.constructor.template])
  ...
  $getProps() {
    ...
    Object.keys(this.constructor.props).forEach(key => {
      let value = this.constructor.props[key];
      if (value instanceof StringParser) {
        props[key] = value;
      } else if (value.type) {
        let equivalent = TYPES_EQUIVALENTS.find(el => el.type === value.type);
        if (equivalent) {
          props[key] = new equivalent.parser(value);
        } else {
          console.warn(
            `Type "${value.type.name}" on property ${key} in "${
              this.constructor.name
            }" isn't a valid built'in property type. Falling back to StringParser`
          );
          props[key] = new StringParser(value);
        }
      } else {
        props[key] = new StringParser(value);
      }
  ...
  $getPropertyToDefine(propAttrName, prop) {
    return {
      get: () => {
        let value = this.hasAttribute(propAttrName)
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    ...
    if (!callback)
      return console.warn(
        `${
          this.constructor.name
        } : No callback named ${callbackName} on property ${name}`
      );

    if (callback) {
      return callback.bind(this)(oldVal, newVal);
    }
  }
